...
-- Table `Payment`.`Daira`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Payment`.`Daira` (
  `idDaira` INT NOT NULL AUTO_INCREMENT,
  `idWilaya` INT NOT NULL,
  `daira` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idDaira`),
  UNIQUE INDEX `idCommune_UNIQUE` (`idDaira` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Payment`.`Commune` (
  ...
  `idDaira` INT NOT NULL,
  `municipal` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idCommune`),
  UNIQUE INDEX `idMunicipal_UNIQUE` (`idCommune` ASC) VISIBLE,
  INDEX `idCommune_idx` (`idDaira` ASC) VISIBLE,
  CONSTRAINT `idDaira`
    FOREIGN KEY (`idDaira`)
    REFERENCES `Payment`.`Daira` (`idDaira`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Payment`.`Address` (
  ...
  INDEX `idMunicipal_idx` (`idCommune` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Payment`.`Order` (
  ...
  `state` ENUM("reserved", "canceled", "waiting", "paid", "pending") NOT NULL,
